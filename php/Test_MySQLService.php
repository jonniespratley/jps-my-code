...
class Test_MySQLService
{
	 ...
	 * @return MySQLService
	 */
	public function Test_MySQLService( $host, $username, $password )
	{
 ...
 * *********************************************************************/
	...
	public function getDatabases()
	{
		...
		while ( $database = mysqli_fetch_assoc ( $databaseSQL ) )
		{
			...
			//Add the tables to the database array
			$databases [] = array ( 
				"aDatabase" => $value, "aType" => "database", "aData" => $key, "aDependencies" => $dependencies, "aStoredProcs" => $storedprocs, "aViews" => $views, "aUserFunctions" => $userFunctions, "aIcon" => "databaseIcon", "aTables" => $tables, "aStatus" => $status, "aSize" => $size 
			);
		}
		sort ( $databases );
 ...
 * ********************************************************************
	 ...
	 * @return [array]
	 */
	private function _getTables( $whatDatabase )
	{
		//table query
		$tableSQL = mysqli_query ( $this->mysqli, "SHOW TABLES FROM $whatDatabase" );
	 ...
	 * @return [array]
	 */
	private function _describeTable( $whatDatabase, $whatTable )
	{
		$fieldSQL = mysqli_query ( $this->mysqli, "SHOW FIELDS FROM $whatDatabase.$whatTable" );
	...
	private function _getTableIndexes( $whatDatabase, $whatTable )
	{
		$sql = "SHOW INDEX FROM $whatDatabase.$whatTable";
	 ...
	 * @return [array]
	 */
	private function _getTableStatus( $whatDatabase )
	{
		$sql = "SHOW TABLE STATUS FROM $whatDatabase";
		
		return $this->_queryToARRAY ( $sql );
	}
	
	private function _getSingleTableStatus( $whatDatabase, $whatTable )
	{
		$sql = "SHOW TABLE STATUS FROM $whatDatabase LIKE '$whatTable'";
...
$svc = new Test_MySQLService ( 'localhost', 'root', 'fred' );
